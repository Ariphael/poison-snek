...
Board::Board(int BDO)
	:
	_BoxDimensionOffset(BDO)
{
}

void Board::DrawBox(Location & BoxLoc, Graphics& gfx, Color c)const
{
	for (int w = (int)BoxLoc.GetPosX(); w <= (int)BoxLoc.GetPosX() + (BoxLoc.GetLocLength() - _BoxDimensionOffset); w++) {
		for (int h = (int)BoxLoc.GetPosY(); h <= (int)BoxLoc.GetPosY() + (BoxLoc.GetLocLength() - _BoxDimensionOffset); h++) {
			gfx.PutPixel(w, h, c);
		}
	}
}

void Board::DrawGameOver(int x, int y, Graphics & gfx) const
{
